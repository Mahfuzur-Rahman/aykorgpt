'use client'

import { useEffect, useRef } from 'react'
import { useRouter } from 'next/navigation'
import { toast } from 'sonner'
import { setSession } from '@/lib/session'

const API = process.env.NEXT_PUBLIC_API_URL || ''
const CLIENT_ID = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID

declare global {
  interface Window {
    google?: any
  }
}

interface GoogleSignInButtonProps {
  label?: string
}

export function GoogleSignInButton({ label = 'Sign in with Google' }: GoogleSignInButtonProps) {
  const router = useRouter()
  const btnRef = useRef<HTMLDivElement>(null)

  useEffect(() => {
    if (!CLIENT_ID || CLIENT_ID.includes('your_google_client_id')) return

    function handleCredentialResponse(response: any) {
      if (!response.credential) return
      fetch(`${API}/api/auth/google`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ credential: response.credential }),
      })
        .then(async (res) => {
          const data = await res.json().catch(() => ({}))
          if (!res.ok) throw new Error(data.detail || 'Google sign in failed.')
          setSession(data)
          toast.success('Signed in with Google!')
          router.push('/chat')
          router.refresh()
        })
        .catch((err) => {
          toast.error(err instanceof Error ? err.message : 'Google authentication failed.')
        })
    }

    const script = document.createElement('script')
    script.src = 'https://accounts.google.com/gsi/client'
    script.async = true
    script.defer = true
    script.onload = () => {
      if (window.google?.accounts?.id && btnRef.current) {
        window.google.accounts.id.initialize({
          client_id: CLIENT_ID,
          callback: handleCredentialResponse,
        })
        window.google.accounts.id.renderButton(btnRef.current, {
          theme: 'outline',
          size: 'large',
          width: 320,
          text: label === 'Sign up with Google' ? 'signup_with' : 'signin_with',
        })
      }
    }
    document.body.appendChild(script)

    return () => {
      if (script.parentNode) script.parentNode.removeChild(script)
    }
  }, [router, label])

  if (!CLIENT_ID || CLIENT_ID.includes('your_google_client_id')) {
    return (
      <button
        type="button"
        onClick={() => toast.info('Google Sign-In is ready! Set NEXT_PUBLIC_GOOGLE_CLIENT_ID in .env.local to enable.')}
        className="w-full flex items-center justify-center gap-3 border border-gray-200 dark:border-gray-700 rounded-xl px-4 py-2.5 text-sm font-medium text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
      >
        <svg className="w-5 h-5" viewBox="0 0 24 24">
          <path
            fill="#4285F4"
            d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
          />
          <path
            fill="#34A853"
            d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
          />
          <path
            fill="#FBBC05"
            d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.06H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.94l2.85-2.22.81-.63z"
          />
          <path
            fill="#EA4335"
            d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 12 1 7.7 1 3.99 3.47 2.18 7.06l3.66 2.84c.87-2.6 3.3-4.52 6.16-4.52z"
          />
        </svg>
        {label}
      </button>
    )
  }

  return <div ref={btnRef} className="flex justify-center w-full min-h-[44px]" />
}
